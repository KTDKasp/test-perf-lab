...
export function useProductFilters() {
  ...
  const page =
    (Number(searchParams.get("currentPage")) as Filters["currentPage"]) || 1;
  const category =
    (searchParams.get("category") as Filters["category"]) || "all";
  const sortProperty =
    (searchParams.get("sortProperty") as Filters["sort"]) || "name";
